...
# 10人全員分の時系列データをプロット
plot((n_burnins+1):(n_burnins+n_timepoints), rep(NA, n_timepoints), type = 'n', xlim = c((n_burnins+1), (n_burnins+n_timepoints)), ylim = range(unlist(time_series_list)), main = "AR(2) Time Series for All Individuals", xlab = "Time", ylab = "Value")

# 10色でプロットするための色を生成
colors <- rainbow(n_people)

for(i in 1:n_people) {
  lines((n_burnins+1):(n_burnins+n_timepoints), time_series_list[[i]], col = colors[i], lwd = 1)
}
```
...
for (i in 1:n_people) {
  ...
  # 各モデルの予測値
  mean_pred_AR1 <- pred_AR1[i, n_burnins:n_timepoints]
  mean_pred_AR2 <- pred_AR2[i, n_burnins:n_timepoints]
  mean_pred_AR3 <- pred_AR3[i, n_burnins:n_timepoints]
