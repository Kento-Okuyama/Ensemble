...
# Initialize a list to store the AR(2) time series data for 10 people
time_series_list <- list()

# Generate data using an independent AR(2) process for each person
set.seed(123)  # Set seed for reproducibility
for(i in 1:n_people) {
  # Set AR(2) parameters (example: phi1=0.5, phi2=-0.3, mu=0, sigma=1)
  # Here, phi1 and phi2 are the autoregressive coefficients, mu is the mean, and sigma is the standard deviation of the noise.
  phi1 <- 0.5
  phi2 <- -0.3
  mu <- 0
  sigma <- 1
  
  # Generate data based on the AR(2) process
  e <- rnorm(n_burnins + n_timepoints, mean = mu, sd = sigma)  # Generate noise
  data <- numeric(n_burnins + n_timepoints)
  data[1] <- e[1]  # Initial value (randomly generated)
  
  # The second point is generated based on the AR(1) model
  data[2] <- mu + phi1 * (data[1] - mu) + e[2]
  
  # Points from the third onwards are generated based on the AR(2) model
