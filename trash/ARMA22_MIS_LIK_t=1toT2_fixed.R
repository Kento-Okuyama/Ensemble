...
n_burnins <- 25      # Number of burn-in time points (discarded)
n_timepoints <- 50   # Number of observed time points
...
# Calculate total log-likelihood for each model
total_log_lik <- function(log_lik) {
  sum(log_lik)  # Sum all log-likelihood values
}

log_lik_AR1_total <- total_log_lik(log_lik_AR1)
log_lik_AR2_total <- total_log_lik(log_lik_AR2)
log_lik_AR3_total <- total_log_lik(log_lik_AR3)
...
weights_df <- data.frame(
  ...
  LL = c(log_lik_AR1_total, log_lik_AR2_total, log_lik_AR3_total)
)
