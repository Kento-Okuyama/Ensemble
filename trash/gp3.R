...
# Create a data frame and visualize it
...
# Hierarchical stacking model (Gaussian process)
...
# 3. Prepare data list for Stan model fitting
data_list <- list(
  N_people = n_individuals,
  N_timepoints = n,
  y = Y,
...
# 4. Fit the hierarchical stacking model
fit_gp_ensemble <- sampling(
  stan_model_gp_ensemble,
  data = data_list,
  iter = 2000,
  warmup = 1000,
  chains = 4,
...
# Calculate MSE for the ensemble model over time
