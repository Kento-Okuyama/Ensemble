...
# Balanced data generation with noise and regime differences
set.seed(123)
...
# Parameters for regime 1 (AR(1)) and regime 2 (Random Walk)
phi_1 <- 0.85  # Slightly less stable AR(1)
mu_1 <- 0.5
sigma_regime_1 <- 0.4  # Increased noise for AR(1)

drift_regime_2 <- 1.0  # Reduced drift for Random Walk
sigma_regime_2 <- 0.8  # Reduced noise for Random Walk

# Transition matrix between regimes
P <- matrix(c(0.5, 0.5,  # Equal probability of switching
              0.5, 0.5), 
            nrow = 2, byrow = TRUE)
...
for (i in 1:n_individuals) {
  ...
  for (t in 2:n) {
    # Sample next regime state
    state[i, t] <- sample(1:2, 1, prob = P[state[i, t-1], ])
    
    # Generate the hidden state X based on the regime
    if (state[i, t] == 1) {
      X[i, t] <- phi_1 * (X[i, t-1] - mu_1) + rnorm(1, 0, sigma_regime_1)  # AR(1)
    } else {
      X[i, t] <- X[i, t-1] + drift_regime_2 + rnorm(1, 0, sigma_regime_2)  # Random Walk
    }
    
    # Observed data Y is noisy version of hidden state X
    Y[i, t] <- X[i, t] + rnorm(1, 0, 0.5)  # Observation noise
  }
}
...
ggplot(df, aes(x = time, y = X, group = individual, color = state)) +
  ...
  labs(title = "Balanced Regime Switching Data",
       x = "Time", y = "X") +
...
stan_model_gp_ensemble <- stan_model(model_code = "
  ...
  model {
    ...
    rho ~ normal(0, 0.3);  // Reduce length scale to allow faster changes
    for (j in 1:N_people) {
      for (k in 1:2) {
...
# Function to calculate RMSE over time for the given model
calculate_rmse_time <- function(y_hat, y_true, start_point = 1) {
  rmse_values <- sapply(start_point:dim(y_hat)[2], function(t) {
    rmse_per_person <- sapply(1:nrow(y_true), function(i) sqrt(mean((y_hat[i, t] - y_true[i, t])^2)))
    return(mean(rmse_per_person))
...
# 1. Calculate RMSE for Random Walk model
rmse_time_rw <- calculate_rmse_time(y_hat_RW, Y)

# 2. Calculate RMSE for AR(1) model
rmse_time_ar1 <- calculate_rmse_time(y_hat_AR1, Y)

# 3. Calculate RMSE for the ensemble model (already done previously)
rmse_time_ensemble <- calculate_rmse_time(y_hat_ensemble, Y)

# Prepare data for plotting RMSE over time for all models
rmse_time_data <- data.frame(
  Time = rep(1:n, 3),
  RMSE = c(rmse_time_rw, rmse_time_ar1, rmse_time_ensemble),
  Model = factor(rep(c("Random Walk", "AR(1)", "Ensemble"), each = n))
)

# Plot RMSE over time for all models
ggplot(rmse_time_data, aes(x = Time, y = RMSE, color = Model, group = Model)) +
  geom_line(size = 1.2) +
  labs(title = "RMSE over Time for Random Walk, AR(1), and Ensemble Models", 
       x = "Time", y = "RMSE") +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 16),  # Centered and larger title
  ...
  scale_color_manual(values = c("Random Walk" = "red", "AR(1)" = "blue", "Ensemble" = "purple"))

# Calculate and print the final RMSE for Random Walk, AR(1), and Ensemble models
final_rmse_rw <- sqrt(mean((y_hat_RW - Y)^2))
final_rmse_ar1 <- sqrt(mean((y_hat_AR1 - Y)^2))
final_rmse_ensemble <- sqrt(mean((y_hat_ensemble - Y)^2))

cat("Final RMSE for Random Walk:", final_rmse_rw, "\n")
cat("Final RMSE for AR(1):", final_rmse_ar1, "\n")
cat("Final RMSE for Ensemble:", final_rmse_ensemble, "\n")
