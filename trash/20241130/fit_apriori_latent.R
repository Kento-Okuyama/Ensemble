fit_apriori_latent <- function(data, iter = 2000, chains = 4) {
  ...
  model {
    ...
    for (t in 1:train_Nt) {
      train_eta[t] ~ normal(mu_wn, sigma_wn);
    }
  ...
  generated quantities {
    real y_pred_wn[train_Nt + test_Nt];
    real eta_pred_wn[train_Nt + test_Nt];
    real log_lik[train_Nt];  // Log-likelihood for PSIS-LOO
    real test_y_pred[test_Nt]; // Predicted values for test data
    real SSE;
    real RMSE;
    ...
    SSE = 0;
    eta_pred_wn[1] = normal_rng(mu_wn, sigma_wn);
    y_pred_wn[1] = normal_rng(eta_pred_wn[1], sigma_m);

    for (t in 2:(train_Nt + test_Nt)) {
      eta_pred_wn[t] = normal_rng(mu_wn, sigma_wn);
      y_pred_wn[t] = normal_rng(eta_pred_wn[t], sigma_m);
    }
    ...
    for (t in 1:test_Nt) {
      test_y_pred[t] = y_pred_wn[train_Nt + t];
      SSE += square(test_y[t] - test_y_pred[t]);
    }
    RMSE = sqrt(SSE / test_Nt);
  }
  ...
  e3 <- apply(eta_pred_wn, 2, mean)
  f1 <- apply(y_pred_ar, 2, mean)
  f2 <- apply(y_pred_ma, 2, mean)
  f3 <- apply(y_pred_wn, 2, mean)
  
  # Combine predictions into a single array
  pred_e <- array(c(e1, e2, e3), dim = c(data$train_Nt + data$test_Nt, 3))
  train_f <- array(c(f1[1:data$train_Nt], f2[1:data$train_Nt], f3[1:data$train_Nt]), dim = c(data$train_Nt, 3))
  test_f <- array(c(f1[(data$train_Nt + 1):(data$train_Nt + data$test_Nt)], f2[(data$train_Nt + 1):(data$train_Nt + data$test_Nt)], f3[(data$train_Nt + 1):(data$train_Nt + data$test_Nt)]), dim = c(data$test_Nt, 3))
