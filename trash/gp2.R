...
data {
  ...
  vector[N_timepoints] y[N_people];  // Observed time series data
}
parameters {
  real mu;                       // Mean of the process
  real<lower=0> sigma;           // Standard deviation of the noise
}
model {
  mu ~ normal(0, 10);            // Prior for the mean
  sigma ~ cauchy(0, 2.5);        // Prior for the standard deviation
  for (j in 1:N_people) {
    for (n in 1:N_timepoints) {
      y[j][n] ~ normal(mu, sigma);  // AR(0) process: independent normal distribution
    }
  }
}
generated quantities {
  vector[N_timepoints] log_lik[N_people];  // Log-likelihood for each observation
  vector[N_timepoints] y_hat[N_people];    // Posterior predictive samples
  for (j in 1:N_people) {
    for (n in 1:N_timepoints) {
      log_lik[j][n] = normal_lpdf(y[j][n] | mu, sigma);  // Log-likelihood
      y_hat[j][n] = normal_rng(mu, sigma);               // Posterior predictive distribution
    }
  }
}
")
...
# Fit Stan models
fit_AR0 <- sampling(stan_model_AR0, data = data_list, iter = 2000, chains = 4)
fit_AR1 <- sampling(stan_model_AR1, data = data_list, iter = 2000, chains = 4)

# Extract posterior predictive samples for each model
...
# Function to calculate mean squared error (MSE) at each time point
...
mse_time_AR0 <- calculate_mse_time(array(y_hat_AR0, dim = c(n_people, n_timepoints)), data_list$y)
...
mse_time_data <- data.frame(
  Time = rep(x_axis, 3),
  MSE = c(mse_time_AR0, mse_time_AR1, mse_time_ensemble),
  Model = factor(rep(c("AR(0)", "AR(1)", "Ensemble"), each = length(x_axis)))
)

# Plot MSE over time with log scale on the y-axis
ggplot(mse_time_data, aes(x = Time, y = MSE, color = Model, group = Model)) +
  ...
  scale_color_manual(values = c("AR(0)" = "gray", "AR(1)" = "gray", "Ensemble" = "purple"))
