---
title: "Bayesian Hierarchical Stacking for ARMA Models"
author: "Your Name"
date: "`r Sys.Date()`"
...
generated quantities {
  vector[N_timepoints] log_lik_AR1[N_people];
  vector[N_timepoints] y_hat_AR1[N_people];
  for (j in 1:N_people) {
    log_lik_AR1[j][1] = 0;
    y_hat_AR1[j][1] = y[j][1];
    for (n in 2:N_timepoints) {
      log_lik_AR1[j][n] = normal_lpdf(y[j][n] | mu + phi1 * (y[j][n-1] - mu), sigma);
      y_hat_AR1[j][n] = normal_rng(mu + phi1 * (y[j][n-1] - mu), sigma);
    }
  }
}
")
...
generated quantities {
  vector[N_timepoints] log_lik_AR2[N_people];
  ...
  for (j in 1:N_people) {
    ...
    y_hat_AR2[j][2] = y[j][2];
    for (n in 3:N_timepoints) {
      log_lik_AR2[j][n] = normal_lpdf(y[j][n] | mu + phi1 * (y[j][n-1] - mu) + phi2 * (y[j][n-2] - mu), sigma);
      y_hat_AR2[j][n] = normal_rng(mu + phi1 * (y[j][n-1] - mu) + phi2 * (y[j][n-2] - mu), sigma);
    }
  }
}
")
...
generated quantities {
  vector[N_timepoints] log_lik_AR3[N_people];
  ...
  for (j in 1:N_people) {
    ...
    y_hat_AR3[j][3] = y[j][3];
    for (n in 4:N_timepoints) {
      log_lik_AR3[j][n] = normal_lpdf(y[j][n] | mu + phi1 * (y[j][n-1] - mu) + phi2 * (y[j][n-2] - mu) + phi3 * (y[j][n-3] - mu), sigma);
      y_hat_AR3[j][n] = normal_rng(mu + phi1 * (y[j][n-1] - mu) + phi2 * (y[j][n-2] - mu) + phi3 * (y[j][n-3] - mu), sigma);
    }
  }
}
")
...
y_hat_AR1 <- apply(extract(fit_AR1, pars = "y_hat_AR1")$y_hat_AR1, c(2,3), mean)
y_hat_AR2 <- apply(extract(fit_AR2, pars = "y_hat_AR2")$y_hat_AR2, c(2,3), mean)
y_hat_AR3 <- apply(extract(fit_AR3, pars = "y_hat_AR3")$y_hat_AR3, c(2,3), mean)
```
...
# Function to calculate mean squared error (MSE) at each time point starting from the 4th data point
