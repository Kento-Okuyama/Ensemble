...
parameters {
  simplex[3] w[N_timepoints]; // Weights for each model at each time point (Dirichlet distributed)
...
model {
  ...
  for (j in 1:N_people) {
    for (t in 1:N_timepoints) {
      real y_pred = w[t, 1] * y_hat_AR1[j, t] + w[t, 2] * y_hat_AR2[j, t] + w[t, 3] * y_hat_AR3[j, t];
