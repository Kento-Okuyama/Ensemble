...
data {
  ...
  int<lower=1, upper=4> regime[N, Nt]; // States at each time step
...
transformed parameters {
  vector[4] w[N, Nt];               // Dynamic weights
  ...
  for (n in 1:N) {
    for (t in 2:Nt) {
      for (j in 1:4) { 
          w[n, t][j] = T[1][j] * w[n, t - 1][1] + T[2][j] * w[n, t - 1][2] + T[3][j] * w[n, t - 1][3] + T[4][j] * w[n, t - 1][4]; 
      }
    }
  }
}
model {
  ...
  for (n in 1:N) {
    for (t in 2:Nt) {
      ...
      y_pred[1] = target_mean + ar_pos * (y[n, t - 1] - target_mean);
      y_pred[2] = target_mean + ar_neg * (y[n, t - 1] - target_mean);
      y_pred[3] = target_mean + ma_coef * (y[n, t - 1] - target_mean);
      y_pred[4] = target_mean;

      // Weighted prediction based on dynamic weights
      weighted_prediction = dot_product(w[n, t], y_pred);

      // Observation model
      y[n, t] ~ normal(weighted_prediction, sigma[k]);
    }
  }
}
generated quantities {
  ...
  for (n in 1:N) {
    for (t in 2:Nt) {
      ...
      weighted_prediction = dot_product(w[n, t], y_pred);
      y_rep[n, t-1] = normal_rng(weighted_prediction, sigma[k]);
      log_lik[n, t-1] = normal_lpdf(y[n, t] | weighted_prediction, sigma[k]);
    }
  }
}
"
