...
# 1. Generate Markov regime-switching data for multiple individuals
set.seed(123)
n <- 30  # Number of time points per individual
n_individuals <- 5  # Number of individuals

# Create matrices to store data for each individual
X <- matrix(0, nrow = n_individuals, ncol = n)  # Hidden states
Y <- matrix(0, nrow = n_individuals, ncol = n)  # Observed data
state <- matrix(0, nrow = n_individuals, ncol = n)  # Regimes (1 or 2)

# Parameters for the two regimes (can be extended for individuals if needed)
alpha_1 <- 0.8  # Autoregressive coefficient for regime 1
beta_1 <- 1.0   # Slope for regime 1
alpha_2 <- -1.0  # Autoregressive coefficient for regime 2
...
for (i in 1:n_individuals) {
  ...
  for (t in 2:n) {
    ...
    if (state[i, t] == 1) {
      ...
      X[i, t] <- alpha_2 * X[i, t-1] + rnorm(1, 0, 1)
    }
...
X_simple <- rstan::extract(fit_simple)$X
...
X_regime <- rstan::extract(fit_regime_multi)$X
...
regime_prob <- rstan::extract(fit_regime_multi)$regime_prob
