...
phi_1 <- 1.2  # Autoregressive coefficient for regime 1
mu_1 <- 0.5
...
for (i in 1:n_individuals) {
  ...
  for (t in 2:n) {
    ...
    if (state[i, t] == 1) {
      X[i, t] <- phi_1 * (X[i, t-1] - mu_1) + rnorm(1, 0, 0.5)  # AR(1) process for regime 1
    } else {
      X[i, t] <- X[i, t-1] + rnorm(1, 0, 0.5)  # Random walk for regime 2
    }
