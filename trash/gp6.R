...
stan_model_AR0 <- stan_model(model_code = "
  ...
  parameters {
    real mu_x;  // mean of the latent state process
    real<lower=0> sigma_x;  // standard deviation of the latent state process
    real<lower=0> sigma_y;  // observation noise
    vector[N_timepoints] x[N_people];  // latent states (one for each time point)
  }
  model {
    // Priors
    mu_x ~ normal(0, 10);  // prior for latent state mean
    sigma_x ~ cauchy(0, 2.5);  // prior for latent state stddev
    sigma_y ~ cauchy(0, 2.5);  // prior for observation noise

    // Latent state model (AR(0), x process is independent over time)
    for (j in 1:N_people) {
      for (n in 1:N_timepoints) {
        x[j][n] ~ normal(mu_x, sigma_x);  // AR(0) for latent state
      }
      // Observation model
