...
def get_kalman_predictions_and_latents(Y, b0, B1, Lambda1, Q, R):
    ...
    P_series = torch.zeros(N, Nt, L1, L1, device=device) # Pの履歴を保存
...
def bps_random_walk_model(y_obs, factors_f):
    ...
    # ▼▼▼【変更点 1/3】torch.diag を torch.diag_embed に変更 ▼▼▼
    ...
    # ▼▼▼【変更点 2/3】torch.diag を torch.diag_embed に変更 ▼▼▼
    ...
    # ▼▼▼【変更点 3/3】torch.diag を torch.diag_embed に変更 ▼▼▼
...
if os.path.exists(BPS_GUIDE_FILE):
    ...
    print("Starting BPS model training with VI...");patience=100;patience_counter=0;best_loss_bps=float('inf');pyro.clear_param_store();pbar=trange(20000)
    for step in pbar:
...
if os.path.exists(RS_KF_MODEL_FILE):
    ...
    start_time_rs=time.time();best_loss_rs=float('inf');best_model_rs_state=None;patience=100;patience_counter=0;absolute_threshold=0.1;print("Starting RS-KF model training...");model_rs=RegimeSwitchingKF(O,initial_belief='informative',init_params=frs_initial_params).to(device);optimizer_rs=torch.optim.Adam(model_rs.parameters(),lr=1e-3);pbar=trange(20000)
    for epoch in pbar:
    ...
    loss_rs=best_loss_rs;duration_rs=time.time()-start_time_rs;print(f"RS-KF model training finished. Duration: {duration_rs:.2f}s");print(f"Saving RS-KF model results to '{RS_KF_MODEL_FILE}'...");results_to_save={'model_state_dict':best_model_rs_state,'loss':loss_rs,'duration':duration_rs};torch.save(results_to_save,RS_KF_MODEL_FILE)
...
# --- 6d. 最終比較テーブル ---
table_width = 110;print("\n"+"="*table_width);print(f"{'Model':<30} | {'Loss':<15} | {'RMSE':<15} | {'Sensitivity':<15} | {'Specificity':<15} |");print("-"*table_width);print(f"{'3-Factor Model':<30} | {loss_3fac:<15.4f} | {rmse_3fac:<15.4f} | {sens_3fac:<15.4f} | {spec_3fac:<15.4f} |");print(f"{'1-Factor Model':<30} | {loss_1fac:<15.4f} | {rmse_1fac:<15.4f} | {sens_1fac:<15.4f} | {spec_1fac:<15.4f} |");print(f"{'BPS Random Walk':<30} | {loss_bps:<15.4f} | {rmse_bps:<15.4f} | {sens_bps:<15.4f} | {spec_bps:<15.4f} |");print(f"{'Regime-Switching KF':<30} | {loss_rs:<15.4f} | {rmse_rs:<15.4f} | {sens_rs:<15.4f} | {spec_rs:<15.4f} |");print("Note: Loss for Baseline/RS-KF is NegLogLikelihood. Loss for BPS is ELBO.");print("      RMSE, Sensitivity, and Specificity are directly comparable metrics.");print("="*table_width)

# ----------------------------------------------------------------
# Part 7: 可視化
# ----------------------------------------------------------------
print("\n--- 7. Visualization of Results ---")
time_points=np.arange(Nt);state1_proportion_actual=(actual_states==1).mean(axis=0);avg_weights_bps_over_time=estimated_weights_bps.transpose(1,0,2).mean(axis=1)

print("\n--- Generating Aggregate Model Comparison Plot ---")
avg_probs_rs=predicted_probs_rs.mean(axis=0);plt.style.use('seaborn-v0_8-whitegrid');fig,(ax1,ax3)=plt.subplots(1,2,figsize=(20,8),sharey=True);fig.suptitle('Model Comparison: BPS Random Walk vs. Regime-Switching KF (Aggregate Performance)',fontsize=20);ax1.plot(time_points,avg_weights_bps_over_time[:,0],'o-',color='royalblue',label='Avg. Weight for 3-Factor Model (State 1)',zorder=3);ax1.plot(time_points,avg_weights_bps_over_time[:,1],'s-',color='firebrick',label='Avg. Weight for 1-Factor Model (State 2)',zorder=3);ax1.set_title('BPS Random Walk Model',fontsize=16);ax1.set_xlabel('Time Point',fontsize=12);ax1.set_ylabel('Estimated Model Weight',fontsize=12);ax1.legend(loc='upper left');ax1.set_ylim(-0.05,1.05);ax2=ax1.twinx();ax2.bar(time_points,state1_proportion_actual,color='grey',alpha=0.2,label='Proportion in State 1 (Actual)');ax2.set_ylabel('Proportion in State 1 (Ground Truth)',fontsize=12);ax2.set_ylim(-0.05,1.05);ax2.grid(False);ax3.plot(time_points,avg_probs_rs[:,0],'o-',color='royalblue',label='Avg. Prob. of Regime 1 (3-Factor)',zorder=3);ax3.plot(time_points,avg_probs_rs[:,1],'s-',color='firebrick',label='Avg. Prob. of Regime 2 (1-Factor)',zorder=3);ax3.set_title('Regime-Switching KF Model',fontsize=16);ax3.set_xlabel('Time Point',fontsize=12);ax3.legend(loc='upper left');ax4=ax3.twinx();ax4.bar(time_points,state1_proportion_actual,color='grey',alpha=0.2,label='Proportion in State 1 (Actual)');ax4.set_ylabel('Proportion in State 1 (Ground Truth)',fontsize=12);ax4.set_ylim(-0.05,1.05);ax4.grid(False);plt.tight_layout(rect=[0,0.03,1,0.95]);plt.savefig("model_comparison_plot.png");print("Aggregate model comparison plot saved to model_comparison_plot.png");plt.show()
...
posterior_samples = predictive(Y_generated, factors_f)
...
for file_idx in range(num_files):
    ...
    for i, ind_id in enumerate(ids_to_plot):
        # BPS Plot
        ax_bps = axes[i, 0]
        ax_bps.plot(time_points, estimated_weights_bps[ind_id, :, 1], 'g-', label='BPS Prob. (State 2)', lw=1.5)
        ax_bps.fill_between(time_points, bps_lower_ci[ind_id, :], bps_upper_ci[ind_id, :], color='green', alpha=0.2, label='95% Credible Interval')
        ax_bps.plot(time_points, actual_states_binary[ind_id, :], 'r--', label='True State', lw=1.5)
        ax_bps.set_title(f'Individual #{ind_id+1} - BPS Random Walk Model', fontsize=10)
        ax_bps.set_ylabel('Prob. of State 2')
        
        # RS-KF Plot
        ax_rs = axes[i, 1]
        ax_rs.plot(time_points, predicted_probs_rs[ind_id, :, 1], 'b-', label='RS-KF Prob. (State 2)', lw=1.5)
        ax_rs.plot(time_points, actual_states_binary[ind_id, :], 'r--', label='True State', lw=1.5)
        ax_rs.set_title(f'Individual #{ind_id+1} - RS-KF Model', fontsize=10)

        # 各ファイルの最初のプロットにのみ凡例を表示
        if i == 0:
    ...
    plt.close(fig) # メモリを解放するためにプロットを閉じる
    ...
    for i, ind_id in enumerate(ids_to_plot):
        ...
        for l in range(L1_state1):
            ...
            if np.any(mask_true_r1):
                ax.plot(time_points[mask_true_r1], eta_true_hist_numpy[ind_id, :, l][mask_true_r1], 'r--', label='True', lw=1.5)
        ...
        if np.any(mask_true_r2):
            ax.plot(time_points[mask_true_r2], eta_true_hist_numpy[ind_id, :, 0][mask_true_r2], 'r--', label='True', lw=1.5)
