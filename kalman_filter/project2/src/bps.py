...
if __name__ == '__main__':
    ...
    # RMSEや感度・特異度の計算は中央値を使って行う
    estimated_weights_bps = np.stack([1 - pi2_median, pi2_median], axis=-1)
    m_factors_np = m_factors.cpu().numpy()
    weights_expanded = np.expand_dims(estimated_weights_bps, axis=2)
    y_pred_bps = np.sum(m_factors_np * weights_expanded, axis=3)
