...
def parse_log_file(log_path, model_type):
    ...
    # ★★★ 修正: FRSとBPSのgammaパラメータを正しくパースするロジック ★★★
...
for i in range(1, NUM_REALIZATIONS + 1):
    ...
    # DGPデータの読み込み
    dgp_data_file = os.path.join(BASE_RESULTS_DIR, MODE_DIR, f'dgp_data_run_{i}', f'simulation_data_{DGP_MODE}.pt')
...
if param_df_list:
    param_df_melted = pd.concat(param_df_list)
else:
