...
class RegimeSwitchingKF(torch.nn.Module):
    ...
    def forward(self, y):
        ...
        for t in range(Nt):
            ...
            # 解決策1: ジッターを少し増やす (例: 1e-6)
            JITTER_VALUE = 1e-6
            F1_jitter = F1 + torch.eye(O, device=device) * JITTER_VALUE
            F2_jitter = F2 + torch.eye(O, device=device) * JITTER_VALUE
...
frs_initial_params = {
    ...
    'R_1fac': torch.diag(torch.ones(O, device=device) * R_VAR), # FRSクラスではR_3facのみ使用されるが、一貫性のため定義
    'gamma_intercept': -2.5 + np.random.randn() * 0.1,
    'gamma_coeffs': (np.random.randn(3) * 0.1).tolist(),
}
...
# --- 観測データに基づく尤度を事前に計算 ---
with torch.no_grad():
    B, Q, R, L1, L2 = final_model._build_matrices()
    ...
    for t in range(Nt):
        ...
        F1 = L1 @ P_pred_series[:,t,:,:] @ L1.T + R
        v2 = Y_generated[:, t, :].unsqueeze(-1) - (L2 @ eta_pred_series[:,t,:,None])
        F2 = L2 @ P_pred_series[:,t,:,:] @ L2.T + R
...
for file_idx in range(num_files):
    ...
    for i, ind_id in enumerate(ids_to_plot):
        ax = axes[i, 0]
        # 中央値を線でプロット
        ax.plot(time_points_vis, pi2_median[ind_id, :], 'g-', label='FRS Prob. (Median)', lw=2)
        # 95%信頼区間をバンドでプロット
        ax.fill_between(time_points_vis, pi2_lower_ci[ind_id, :], pi2_upper_ci[ind_id, :], color='g', alpha=0.2, label='95% Confidence Interval')
        # 真値をプロット
