...
def bps_filter_model(y_obs, m_factors, v_factors, eta_3fac, P_3fac, log_liks_stacked):
    ...
    gamma_intercept = pyro.sample("gamma_intercept", dist.Normal(torch.tensor(-2.5, device=device).float(), torch.tensor(0.1, device=device).float()))
    gamma_coeffs = pyro.sample("gamma_coeffs", dist.Normal(torch.tensor(0.0, device=device).float(), torch.tensor(0.1, device=device).float()).expand([L1_state1]).to_event(1))
    p21 = 0.0
