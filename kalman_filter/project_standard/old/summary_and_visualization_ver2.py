...
def parse_log_file(log_path, model_type):
    ...
    if model_type == 'VAR':
        ...
        if (m := re.search(r"-- Estimated b0 \(3-factor\) --\s*\n\s*(.*?)\n", content, re.DOTALL)):
            metrics['b0_3fac'] = parse_vector_from_log(m.group(1))
        if (m := re.search(r"-- Estimated b0 \(1-factor\) --\s*\n\s*(.*?)\n", content, re.DOTALL)):
            metrics['b0_1fac'] = parse_vector_from_log(m.group(1))
        ...
        # ★★★ 修正箇所: Rを状態ごとにパース ★★★
        if (m := re.search(r"-- Estimated R diag \(State 1\) --\s*\n\s*(.*?)\n", content, re.DOTALL)):
            metrics['R_diag_s1'] = parse_vector_from_log(m.group(1))
        if (m := re.search(r"-- Estimated R diag \(State 2\) --\s*\n\s*(.*?)\n", content, re.DOTALL)):
            metrics['R_diag_s2'] = parse_vector_from_log(m.group(1))
...
true_params = {
    ...
    'b0_1fac': b0_1fac_true,           # ★ b0の真値を修正 (DGPに合わせる)
    **true_params_common
}
...
# プロット4: b0 Intercepts
# Filter only for the relevant parameter groups
b0_data = params_df[params_df['ParamGroup'].isin(['b0_3fac', 'b0_1fac'])].copy()

# A simpler, more explicit function to create plot labels and sort keys
def get_b0_plot_labels_and_sort_key(row):
    param_group = row['ParamGroup']
    param_name = row['ParamName']
    
    # Handle parameters from the 3-factor models (VAR_3fac, FRS)
    if param_group == 'b0_3fac':
        if '(0,)' in param_name: return 'b0_state1(0)', 0
        if '(1,)' in param_name: return 'b0_state1(1)', 1
        if '(2,)' in param_name: return 'b0_state1(2)', 2
        
    # Handle parameters from the 1-factor models (VAR_1fac, FRS)
    elif param_group == 'b0_1fac':
        if '(0,)' in param_name: return 'b0_state2(0)', 3
        
    return None, None # Return None if no match

b0_data[['PlotParamName', 'sort_key']] = b0_data.apply(get_b0_plot_labels_and_sort_key, axis=1, result_type='expand')
b0_data.dropna(subset=['PlotParamName'], inplace=True)
b0_data = b0_data.sort_values(by='sort_key')
y_order_b0 = b0_data['PlotParamName'].unique().tolist()
...
# ★ プロット4: b0 Intercepts
ax3 = axes[1, 1]
sns.boxplot(data=b0_data, y='PlotParamName', x='Value', hue='Model', ax=ax3, palette=color_palette, 
            hue_order=['FRS', 'VAR_3fac', 'VAR_1fac'], order=y_order_b0, boxprops=dict(alpha=0.6))
true_b0_vals = {
    'b0_state1(0)': true_params['b0_3fac'][0], 
    'b0_state1(1)': true_params['b0_3fac'][1], 
    'b0_state1(2)': true_params['b0_3fac'][2], 
    'b0_state2(0)': true_params['b0_1fac'][0]
}
...
for param_name, true_val in true_b0_vals.items():
    if param_name in y_order_b0:
        ...
        ax3.axvline(true_val, ymin=(y_pos + 0.05) / num_cats_b0, ymax=(y_pos + 0.95) / num_cats_b0, color='black', ls='--', lw=1.5)
...
# ★★★ Rのプロットを状態ごとに分割 ★★★
fig_meas, axes_meas = plt.subplots(1, 2, figsize=(20, 10))
fig_meas.suptitle('Measurement Model: R (Measurement Noise) Estimates', fontsize=20)

# --- State 1 / 3-Factor ---
ax_r1 = axes_meas[0]
r_s1_data = params_df[params_df['ParamGroup'].isin(['R_3fac_diag', 'R_diag_s1'])].copy()
# FRSのモデル名を変更してVARと区別
r_s1_data['Model'] = r_s1_data.apply(lambda row: 'FRS' if row['ParamGroup'] == 'R_diag_s1' else row['Model'], axis=1)

if not r_s1_data.empty:
    r_s1_data['sort_key'] = r_s1_data['ParamName'].apply(lambda x: int(re.search(r'\d+', x).group()))
    r_s1_data = r_s1_data.sort_values(by='sort_key')
    sns.boxplot(data=r_s1_data, y='ParamName', x='Value', hue='Model', ax=ax_r1, palette=color_palette, hue_order=['VAR_3fac', 'FRS'], boxprops=dict(alpha=0.6))
    ax_r1.axvline(true_params['R_diag'][0], color='black', ls='--', lw=1.5, label='True Value')
    ax_r1.set_title('R Diagonals (State 1 / 3-Factor)')
    ax_r1.legend()

# --- State 2 / 1-Factor ---
ax_r2 = axes_meas[1]
r_s2_data = params_df[params_df['ParamGroup'].isin(['R_1fac_diag', 'R_diag_s2'])].copy()
r_s2_data['Model'] = r_s2_data.apply(lambda row: 'FRS' if row['ParamGroup'] == 'R_diag_s2' else row['Model'], axis=1)

if not r_s2_data.empty:
    r_s2_data['sort_key'] = r_s2_data['ParamName'].apply(lambda x: int(re.search(r'\d+', x).group()))
    r_s2_data = r_s2_data.sort_values(by='sort_key')
    sns.boxplot(data=r_s2_data, y='ParamName', x='Value', hue='Model', ax=ax_r2, palette=color_palette, hue_order=['VAR_1fac', 'FRS'], boxprops=dict(alpha=0.6))
    ax_r2.axvline(true_params['R_diag'][0], color='black', ls='--', lw=1.5, label='True Value')
    ax_r2.set_title('R Diagonals (State 2 / 1-Factor)')
    ax_r2.legend()

plt.tight_layout(rect=[0, 0.03, 1, 0.96])
plt.savefig(os.path.join(SUMMARY_DIR, 'measurement_R_parameters_comparison.png'))
plt.close(fig_meas)
print("Measurement parameter (R) comparison plot saved.")


# --- Lambdaパラメータのプロット (変更なし) ---
...
if not lambda_3fac_data.empty:
    ...
    for i, label in enumerate(y_labels):
        true_val_index = int(re.search(r'\((\d+),\)', label).group(1))
        ax.axvline(true_vals[true_val_index], ymin=(i + 0.1) / len(y_labels), ymax=(i + 0.9) / len(y_labels), color='black', ls='--', lw=1)
...
axes_lambda[1].legend(handles=handles, loc='upper right')
