...
for i in trange(N,desc="Generating data for each person"):
    ...
    for t in range(Nt):
        ...
        if current_state==1:
            eta_t = b0_true_state1 + (B1_true_state1 @ eta_history_i) + q_dist_s1.sample().reshape(L1_state1, 1)
            y_mean_t=Lambda1_true_state1@eta_t
        else:
            if not has_switched:
                eta_history_i=torch.tensor([eta_history_i.mean()],device=device).reshape(L1_state2,1)
                has_switched=True
            eta_t = b0_true_state2 + (B1_true_state2 @ eta_history_i) + q_dist_s2.sample().reshape(L1_state2, 1)
