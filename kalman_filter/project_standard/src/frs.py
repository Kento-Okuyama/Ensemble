...
print("\n--- 2. Defining Regime-Switching Kalman Filter (Mixture) Model ---")
...
class RegimeSwitchingKF(torch.nn.Module):
    def __init__(self, O, L1_state1, L1_state2, init_params):
        ...
        # ★★★ 修正: b0 パラメータを追加 ★★★
        self.b0_state1 = torch.nn.Parameter(init_params['b0_3fac'].clone().float())
        self.b0_state2 = torch.nn.Parameter(init_params['b0_1fac'].clone().float())
    ...
    def forward(self, y):
        ...
        for t in range(Nt):
            ...
            p21 = 0.0
            p22 = 1.0 - p21
            # ★★★ 修正: 分離したb0を計算の直前に結合して使用 ★★★
            b0_combined = torch.cat([self.b0_state1, self.b0_state2]).unsqueeze(-1)
            eta_pred_1 = b0_combined + B @ eta_tm1
            ...
            P_upd_1 = I_minus_K1L1 @ P_pred_1 @ I_minus_K1L1.transpose(-1, -2) + K1 @ R1 @ K1.transpose(-1, -2)
            I_minus_K2L2 = I_mat - K2 @ L2
            P_upd_2 = I_minus_K2L2 @ P_pred_1 @ I_minus_K2L2.transpose(-1, -2) + K2 @ R2 @ K2.transpose(-1, -2)
...
def train_frs_model(Y_data, init_params, cache_file):
    if os.path.exists(cache_file):
        print(f"Loading cached Mixture model from '{cache_file}'...")
        return torch.load(cache_file, map_location=device, weights_only=False)
    print("\n--- 3. Training Mixture Model ---")
    ...
    pbar = trange(10000, desc="Training Mixture Model")
    for epoch in pbar:
        optimizer.zero_grad()
        loss = model(Y_data)
        if torch.isnan(loss):
    ...
    print(f"Mixture model training finished. Duration: {duration:.2f}s")
    results = {
    ...
    print(f"Mixture model results saved to '{cache_file}'.")
    return results

# ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
# 修正箇所: FRSの初期値をVARモデルの初期値と一致させるように変更
# ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
print("    -> Defining initial parameters for Mixture Model based on DGP true values + noise (to mirror var.py's initialization)...")
...
# VARモデルの初期化ロジックを模倣
frs_initial_params = {
    'b0_3fac': b0_true_state1.clone() + torch.randn_like(b0_true_state1) * 0.1,
    'b0_1fac': b0_true_state2.clone() + torch.randn_like(b0_true_state2) * 0.1,
...
# ----------------------------------------------------------------
print("\n--- 4. Evaluating Final Mixture Model ---")
...
print("\n--- Mixture Modeling Performance Summary ---")
table_width = 100
print("=" * table_width)
...
print("NOTE: Mixture modeling estimated parameters for p11 are inverted to compare with true parameters for p12.")
...
# ★★★ 修正: b0の推定結果を出力 ★★★
b0_3fac_est = final_model.b0_state1.detach().cpu().numpy()
b0_1fac_est = final_model.b0_state2.detach().cpu().numpy()
print(f"\n-- Estimated b0 (3-factor) --\n  {np.round(b0_3fac_est, 3)}")
print(f"-- True b0 (3-factor) --\n  {np.round(b0_true_state1.cpu().numpy(), 3)}")
print(f"\n-- Estimated b0 (1-factor) --\n  {np.round(b0_1fac_est, 3)}")
print(f"-- True b0 (1-factor) --\n  {np.round(b0_true_state2.cpu().numpy(), 3)}")
...
ax1.set_title(f'Mixture Modeling - Aggregate Performance ({DGP_MODE})', fontsize=16)
...
color_palette = {'Bayesian Stacking': 'steelblue', 'Mixture Modeling': 'lightcoral', 'VAR(3)': 'mediumseagreen', 'VAR(1)': 'goldenrod'}
...
for file_idx in range(num_files):
    ...
    fig.suptitle(f'Mixture Modeling Individual State Probabilities (Individuals {start_idx+1}-{end_idx})', fontsize=16)
    for i, ind_id in enumerate(ids_to_plot):
        ax = axes[i, 0]
        # ★★★ CHANGE 1: Use the Mixture model color from the palette ★★★
        ax.plot(time_points_vis, filtered_probs_frs[ind_id, :, 1], color=color_palette['Mixture Modeling'], ls='-', label='Mixture Modeling Prob. (State 2)', lw=2)
        # ★★★ CHANGE 2: Change the true state line to black ★★★
...
print(f"\Mixture analysis complete. Log saved to '{log_filename}'")