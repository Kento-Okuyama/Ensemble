...
# --- コマンドライン引数の解析 ---
parser = argparse.ArgumentParser(description='Run Bayesian Stacking analysis with specified parameters.')
...
def bps_filter_model(y_obs, m_factors, v_factors, eta_3fac, log_liks_stacked):
    # この関数は m_factors と v_factors を直接は使いませんが、
    # 呼び出し側とのインターフェースを保つために引数として残します。
    device = y_obs.device
    N, Nt, O = y_obs.shape
    L1_state1 = eta_3fac.shape[-1]
    
    # --- 変更なし: パラメータのサンプリング ---
    gamma_intercept = pyro.sample("gamma_intercept", dist.Normal(torch.tensor(-2.0, device=device).float(), torch.tensor(0.5, device=device).float()))
    gamma_coeffs = pyro.sample("gamma_coeffs", dist.Normal(torch.tensor(0.5, device=device).float(), torch.tensor(0.5, device=device).float()).expand([L1_state1]).to_event(1))
    p21 = 0.0
    ...
    with pyro.plate("individuals", N):
        ...
        for t in range(Nt):
            ...
            # log_denominatorがこの時刻tにおける周辺対数尤度
    ...
    # --- 追加: ループの外で、系列全体の対数尤度を一度だけPyroに渡す ---
    pyro.factor("log_likelihood", total_log_lik)

def get_bps_transition_trajectories(posterior_samples, eta_3fac):
    print("  -> Calculating Bayesian Stacking trajectories using median parameters from MCMC samples...")
...
if __name__ == '__main__':
    ...
    print("--- 0. Bayesian Stacking Model Setup ---")
    ...
    # FRSのログを読み込み、最適化ステップ数を取得
    num_bps_steps = 500
    num_bps_burnin = 500

    # ----------------------------------------------------------------
    # Part 2: Bayesian Stackingモデルの事前計算
    # ----------------------------------------------------------------
    print("\n--- 2. Pre-calculating inputs for Bayesian Stacking model ---")
    with torch.no_grad():
        # ★★★ 修正点: 入力データをfloatに変換 ★★★
    ...
    # ----------------------------------------------------------------
    # Part 3: MCMCによるBayesian Stackingモデルの学習
    # ----------------------------------------------------------------
    print("\n--- 3. Training Bayesian Stacking Model with MCMC ---")
    duration_bps = 0.0
    if os.path.exists(BPS_MCMC_RESULTS_FILE):
        print(f"Loading pre-computed Bayesian Stacking MCMC results from '{BPS_MCMC_RESULTS_FILE}'...")
        ...
        print("Starting Bayesian Stacking model inference with MCMC (NUTS)...")
        ...
        mcmc = MCMC(nuts_kernel, num_samples=num_bps_steps, warmup_steps=num_bps_burnin, num_chains=4) # warmupを0に、サンプル数をFRSのステップ数に設定
        ...
        print(f"Saving Bayesian Stacking MCMC results to '{BPS_MCMC_RESULTS_FILE}'...")
    ...
    # ----------------------------------------------------------------
    print("\n--- 4. Evaluating Final Bayesian Stacking Model (using full posterior) ---")
    ...
    # ★★★ FIX: 削除されていた尤度計算と、新しい軌道計算を両立させる ★★★
    all_pi2_trajectories = np.zeros((num_samples, N, Nt))
    all_log_liks = [] # 尤度を保存するリストを復活
    eta_3fac_np = eta_3fac.cpu().numpy()

    for i in trange(num_samples, desc="Processing MCMC samples for trajectories"):
        gamma_intercept_sample = posterior_intercepts[i]
        gamma_coeffs_sample = posterior_coeffs[i]
        pi2_prev_updated = np.zeros(N)
        
        # このサンプルでの状態確率と対数尤度を一時的に保存する配列
        pi2_trajectories_sample = np.zeros((N, Nt))
        log_lik_per_time_sample = np.zeros((N, Nt))

        for t in range(Nt):
            if t > 0:
                logit_p12_t = gamma_intercept_sample + (eta_3fac_np[:, t-1, :] * gamma_coeffs_sample).sum(axis=-1)
                p12_t = 1 / (1 + np.exp(-logit_p12_t))
                pi2_t_predicted = pi2_prev_updated * (1 - 0.0) + (1 - pi2_prev_updated) * p12_t
            else:
                pi2_t_predicted = pi2_prev_updated
            
            log_numerator_s2 = log_liks_stacked[:, t, 1] + np.log(pi2_t_predicted + 1e-9)
            log_numerator_s1 = log_liks_stacked[:, t, 0] + np.log(1 - pi2_t_predicted + 1e-9)
            ...
            log_lik_per_time_sample[:, t] = log_denominator # 各時点の周辺対数尤度
            
            pi2_prev_updated = pi2_t_updated

        all_pi2_trajectories[i, :, :] = pi2_trajectories_sample
        all_log_liks.append(log_lik_per_time_sample.sum()) # このサンプルの合計対数尤度を追加
    ...
    print("\n--- Bayesian Stacking Model Performance Summary ---")
    table_width = 100
    print("=" * table_width)
    ...
    ax1.set_title(f'Bayesian Stacking Regression Model - Aggregate Performance ({DGP_MODE})', fontsize=16)
    ...
    for file_idx in range(num_files):
        ...
        fig.suptitle(f'Bayesian Stacking Individual State Probabilities (Individuals {start_idx+1}-{end_idx})', fontsize=16)
        
        for i, ind_id in enumerate(ids_to_plot):
            ax = axes[i, 0]
            # ★★★ CHANGE 1: Use the Bayesian Stacking color from the palette for the median line ★★★
            ax.plot(time_points_vis, pi2_median[ind_id, :], color=color_palette['Bayesian Stacking'], ls='-', label='Bayesian Stacking Prob. (Median)', lw=2)
            # ★★★ CHANGE 2: Use the Bayesian Stacking color for the credible interval fill ★★★
            ax.fill_between(time_points_vis, pi2_lower_ci[ind_id, :], pi2_upper_ci[ind_id, :], color=color_palette['Bayesian Stacking'], alpha=0.2, label='95% Credible Interval')
    ...
    print(f"\Bayesian Stacking analysis complete. Log saved to '{log_filename}'")
    sys.stdout.close()
