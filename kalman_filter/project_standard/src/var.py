...
if DGP_MODE == 'IMPLEMENT':
    ...
    b0_true_state2 = torch.tensor([0.37], device=device).unsqueeze(1)
    ...
    # 論文 Figure 2, p.8 より
    b0_true_state1 = torch.tensor([0.0, 0.0, 0.0], device=device).unsqueeze(1)
    b0_true_state2 = torch.tensor([0.39], device=device).unsqueeze(1)
...
def fit_model_full_data(model_type, Y_data, dgp_params, cache_file, device):
    ...
    if model_type == '3-Factor':
        ...
        b0 = (b0_true_state1.clone() + torch.randn_like(b0_true_state1) * 0.1).requires_grad_(True)
        ...
        b0 = (b0_true_state2.clone() + torch.randn_like(b0_true_state2) * 0.1).requires_grad_(True)
...
print(f"  b0 (Intercept): Estimated = {np.round(p3_full['b0'].squeeze().cpu().numpy(), 3)}, True = {np.round(b0_true_state1.squeeze().cpu().numpy(), 3)}")
...
print(f"  b0 (Intercept): Estimated = {np.round(p1_full['b0'].squeeze().cpu().numpy(), 3)}, True = {np.round(b0_true_state2.squeeze().cpu().numpy(), 3)}")
