...
class RegimeSwitchingKF(torch.nn.Module):
    def __init__(self, O, L1_state1, L1_state2, init_params):
        ...
        # ★★★ 修正: b0 を単一のパラメータとして扱う ★★★
        self.b0 = torch.nn.Parameter(init_params['b0'].clone().float())
        ...
        # ★★★ 修正: Rを状態ごとに推定 ★★★
        self.log_r_diag_s1 = torch.nn.Parameter(torch.log(torch.diag(init_params['R_3fac'])).float())
        self.log_r_diag_s2 = torch.nn.Parameter(torch.log(torch.diag(init_params['R_1fac'])).float())
    ...
    def _build_matrices(self):
        ...
        # ★★★ 修正: 2つのR行列を構築 ★★★
        R1 = torch.diag(torch.exp(self.log_r_diag_s1))
        R2 = torch.diag(torch.exp(self.log_r_diag_s2))        
        ...
        return B, Q, R1, R2, L1, L2
    
    # --- Part 2内のforwardメソッド全体を置き換え ---
    def forward(self, y):
        y = y.float()
        N, Nt, O = y.shape
        B, Q, R1, R2, L1, L2 = self._build_matrices()
        
        self.filtered_probs = torch.zeros(N, Nt, 2, device=device)
        self.predicted_y = torch.zeros(N, Nt, O, device=device)
        self.eta_filtered = torch.zeros(N, Nt, self.L, device=device)
        self.P_filtered = torch.zeros(N, Nt, self.L, self.L, device=device)
        ...
        for t in range(Nt):
            # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
            # 修正箇所: 遷移確率の計算にetaの不確かさ(P)を考慮
            # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
            if t > 0:
                # BPSと同様の解析的近似を適用
                kappa = torch.tensor(np.sqrt(np.pi / 8), device=device, dtype=torch.float32)
                
                # 状態1に関連するetaの平均と共分散を取得
                eta_hat_prev = eta_tm1[:, :self.L1_state1, :].squeeze(-1)
                P_prev = P_tm1[:, :self.L1_state1, :self.L1_state1]
                
                # 線形予測子 z = γ'η の平均と分散を計算
                mu_z = self.gamma_intercept + torch.einsum('bi,i->b', eta_hat_prev, self.gamma_coeffs)
                sigma2_z = torch.einsum('i,bij,j->b', self.gamma_coeffs, P_prev, self.gamma_coeffs)
                
                # 解析的近似式で遷移確率の期待値を計算
                arg = (kappa * mu_z) / torch.sqrt(1 + (kappa**2) * torch.abs(sigma2_z))
                p11 = 0.5 * (1.0 + torch.erf(arg / math.sqrt(2.0)))
            else:
                p11 = torch.ones(N, device=device) * 0.99 # t=0では初期値を使用
            ...
            # ★★★ 修正: 予測ステップに b0 を追加 ★★★
            eta_pred_1 = self.b0.unsqueeze(-1) + B @ eta_tm1            
            P_pred_1 = B @ P_tm1 @ B.T + Q

            # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
            # 修正箇所: P_pred_1を強制的に対称化して安定させる
            # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
            P_pred_1 = 0.5 * (P_pred_1 + P_pred_1.transpose(-1, -2))
            
            # ★★★ 修正: 状態ごとのRを使用 ★★★
            v1 = y[:, t, :].unsqueeze(-1) - L1 @ eta_pred_1
            F1 = L1 @ P_pred_1 @ L1.T + R1
            v2 = y[:, t, :].unsqueeze(-1) - L2 @ eta_pred_1
            F2 = L2 @ P_pred_1 @ L2.T + R2
            ...
            prob_pred_s1 = prob_tm1[:, 0] * p11 + prob_tm1[:, 1] * p21
            prob_pred_s2 = prob_tm1[:, 0] * p12 + prob_tm1[:, 1] * p22
            
            numerator_s1 = lik_s1 * prob_pred_s1
            numerator_s2 = lik_s2 * prob_pred_s2
            ...
            P_upd_1 = (I_mat - K1 @ L1) @ P_pred_1 @ (I_mat - K1 @ L1).transpose(-1, -2) + K1 @ R1 @ K1.transpose(-1, -2)
            P_upd_2 = (I_mat - K2 @ L2) @ P_pred_1 @ (I_mat - K2 @ L2).transpose(-1, -2) + K2 @ R2 @ K2.transpose(-1, -2)
            ...
            self.eta_filtered[:, t, :] = eta_t.squeeze(-1)
            self.P_filtered[:, t, :, :] = P_t
...
# ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
# 修正箇所: FRSの初期値をVARモデルの初期値と一致させるように変更
# ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
print("    -> Defining initial parameters for FRS based on DGP true values + noise (to mirror var.py's initialization)...")

# DGPの真値をvar.pyから再定義
if DGP_MODE == 'IMPLEMENT':
    # 論文 Figure 2, p.8 より
    b0_true_state1 = torch.tensor([0.0, 0.0, 0.0], device=device)
    b0_true_state2 = torch.tensor([0.37], device=device)
    ...
    # 論文 Figure 2, p.8 より
    b0_true_state1 = torch.tensor([0.0, 0.0, 0.0], device=device)
    b0_true_state2 = torch.tensor([0.39], device=device)
...
b0_true_combined = torch.cat([b0_true_state1, b0_true_state2])

Q1_VAR, Q2_VAR, R_VAR = 0.5, 0.1, 0.5
lambda1_true_values_state1 = torch.tensor([1.2,0.8,1.1,0.9,1.3,0.7], device=device)
lambda1_true_values_state2 = torch.tensor([1.2,0.8,1.0,1.1,0.9,1.0,1.3,0.7], device=device)

# VARモデルの初期化ロジックを模倣
frs_initial_params = {
    'b0': b0_true_combined.clone() + torch.randn_like(b0_true_combined) * 0.1,
    'B1_3fac': B1_true_state1.clone() + torch.randn_like(B1_true_state1) * 0.1,
    'B1_1fac': B1_true_state2.clone() + torch.randn_like(B1_true_state2) * 0.1,
    'lambda_free_3fac': lambda1_true_values_state1.clone() + torch.randn_like(lambda1_true_values_state1) * 0.1,
    'lambda_free_1fac': lambda1_true_values_state2.clone() + torch.randn_like(lambda1_true_values_state2) * 0.1,
    'Q_3fac': torch.diag(torch.ones(3, device=device) * Q1_VAR),
    'Q_1fac': torch.diag(torch.ones(1, device=device) * Q2_VAR),
    'R_3fac': torch.diag(torch.ones(O, device=device) * R_VAR),
    'R_1fac': torch.diag(torch.ones(O, device=device) * R_VAR), 
    'gamma_intercept': -2.0 + np.random.randn() * 0.5,
    'gamma_coeffs': (0.5 + np.random.randn(3) * 0.5).tolist(),
}
...
# まず一度実行して、etaとPの時系列を取得
with torch.no_grad():
    _ = final_model(Y_generated)
    eta_hat_series = final_model.eta_filtered.cpu().numpy()
    P_series = final_model.P_filtered.cpu().numpy()
    y_preds_frs_point = final_model.predicted_y.cpu()
    filtered_probs_point = final_model.filtered_probs.cpu().numpy()
...
with torch.no_grad():
    # ★★★ 修正: 2つのRを受け取る ★★★
    B, Q, R1, R2, L1, L2 = final_model._build_matrices()    
    eta_pred_series = (B @ final_model.eta_filtered.transpose(1, 2)).transpose(1, 2)
    P_pred_series = B @ final_model.P_filtered @ B.transpose(-1, -2) + Q
    
    lik_s1 = torch.zeros(N, Nt, device=device)
    lik_s2 = torch.zeros(N, Nt, device=device)
    for t in range(Nt):
        # ★★★ 修正: 状態ごとのRを使用 ★★★
        v1 = Y_generated[:, t, :].unsqueeze(-1) - (L1 @ eta_pred_series[:,t,:,None])
        F1 = L1 @ P_pred_series[:,t,:,:] @ L1.T + R1
        v2 = Y_generated[:, t, :].unsqueeze(-1) - (L2 @ eta_pred_series[:,t,:,None])
        F2 = L2 @ P_pred_series[:,t,:,:] @ L2.T + R2
        lik_s1[:, t] = torch.exp(MultivariateNormal(torch.zeros(O, device=device), F1 + torch.eye(O, device=device)*1e-6).log_prob(v1.squeeze(-1)))
        lik_s2[:, t] = torch.exp(MultivariateNormal(torch.zeros(O, device=device), F2 + torch.eye(O, device=device)*1e-6).log_prob(v2.squeeze(-1)))
    lik_s1_np = lik_s1.cpu().numpy()
    lik_s2_np = lik_s2.cpu().numpy()

# --- モンテカルロ法でUncertainty Intervalを計算 ---
print("  -> Calculating uncertainty intervals from eta's covariance...")
num_samples = 500
all_pi2_trajectories = np.zeros((num_samples, N, Nt))
gamma_intercept = final_model.gamma_intercept.item()
gamma_coeffs = final_model.gamma_coeffs.detach().cpu().numpy()

for i in trange(num_samples, desc="Simulating trajectories"):
    eta_sample_series = np.zeros((N, Nt, final_model.L1_state1))
    for n in range(N):
        for t in range(Nt):
            mean = eta_hat_series[n, t, :final_model.L1_state1]
            cov = P_series[n, t, :final_model.L1_state1, :final_model.L1_state1]
            # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
            # 修正箇所: covを対称化し、ジッターを加えて数値的安定性を確保
            # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
            cov = 0.5 * (cov + cov.T)
            cov += np.eye(cov.shape[0]) * 1e-8 # 対角成分に微小な値を追加

            try:
                eta_sample_series[n, t, :] = np.random.multivariate_normal(mean, cov, size=1)
            except np.linalg.LinAlgError:
                eta_sample_series[n, t, :] = mean
    
    pi_prev = np.array([0.99, 0.01])
    for t in range(Nt):
        if t > 0:
            eta_s1_prev = eta_sample_series[:, t-1, :]
            logit_p11 = gamma_intercept + (eta_s1_prev * gamma_coeffs).sum(axis=-1)
            p11 = 1 / (1 + np.exp(-logit_p11))
            p12 = 1 - p11; p21 = 0.0; p22 = 1.0
            
            pi_predicted = np.zeros((N, 2))
            pi_predicted[:, 0] = pi_prev[:, 0] * p11 + pi_prev[:, 1] * p21
            pi_predicted[:, 1] = pi_prev[:, 0] * p12 + pi_prev[:, 1] * p22
        else:
            pi_predicted = np.tile(pi_prev, (N, 1))

        # ★★★ FIX: ここで尤度を使った更新を行うのが正しいステップ ★★★
        numerator_s1 = lik_s1_np[:, t] * pi_predicted[:, 0]
        numerator_s2 = lik_s2_np[:, t] * pi_predicted[:, 1]
        marginal_lik = numerator_s1 + numerator_s2 + 1e-9
        
        pi_updated = np.zeros((N, 2))
        pi_updated[:, 0] = numerator_s1 / marginal_lik
        pi_updated[:, 1] = numerator_s2 / marginal_lik

        all_pi2_trajectories[i, :, t] = pi_updated[:, 1]
        pi_prev = pi_updated
...
# --- サマリー統計の表示 ---
rmse_frs = calculate_rmse(Y_generated.cpu(), y_preds_frs_point)
sens_frs, spec_frs = calculate_sens_spec(actual_states, filtered_probs_point[:, :, 1])
...
final_B, final_Q, final_R1, final_R2, _, _ = final_model._build_matrices()
...
R1_diag_est = torch.diag(final_R1).detach().cpu().numpy()
R2_diag_est = torch.diag(final_R2).detach().cpu().numpy()

# ★★★ 修正: b0の推定結果と真値を比較 ★★★
b0_est = final_model.b0.detach().cpu().numpy()
print(f"\n-- Estimated b0 (Intercept) --\n  {np.round(b0_est, 3)}")
print(f"-- True b0 (Intercept) --\n  {np.round(b0_true_combined.cpu().numpy(), 3)}")
...
# ★★★ 修正: Rのログ出力を状態ごとに変更 ★★★
print(f"\n-- Estimated R diag (State 1) --\n  {np.round(R1_diag_est, 3)}")
print(f"-- Estimated R diag (State 2) --\n  {np.round(R2_diag_est, 3)}")
print(f"-- True R diag --\n  {np.round(np.ones(O) * R_VAR, 3)}")
# =============================================================================
...
for file_idx in range(num_files):
    ...
    for i, ind_id in enumerate(ids_to_plot):
        ax = axes[i, 0]
        # ★★★ 修正1: FRSの色をカラーパレットから取得 ★★★
        # 中央値を線でプロット
        ax.plot(time_points_vis, pi2_median[ind_id, :], color=color_palette['FRS'], ls='-', label='FRS Prob. (Median)', lw=2)
        # 95%信頼区間をバンドでプロット
        ax.fill_between(time_points_vis, pi2_lower_ci[ind_id, :], pi2_upper_ci[ind_id, :], color=color_palette['FRS'], alpha=0.2, label='95% Confidence Interval')
