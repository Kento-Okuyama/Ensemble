...
if __name__ == '__main__':
    ...
    for file_idx in range(num_files):
        ...
        for i, ind_id in enumerate(ids_to_plot):
            ax = axes[i, 0]
            # 中央値を線でプロット
            ax.plot(time_points_vis, pi2_median[ind_id, :], 'g-', label='BPS Prob. (Median)', lw=2)
            # 95%信用区間をバンドでプロット
            ax.fill_between(time_points_vis, pi2_lower_ci[ind_id, :], pi2_upper_ci[ind_id, :], color='g', alpha=0.2, label='95% Credible Interval')
            # 真値をプロット
