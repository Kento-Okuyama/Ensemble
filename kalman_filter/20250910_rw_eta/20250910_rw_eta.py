...
# +【今回の改変】BPSモデルがKalman Filterの予測分布(eta, F)を考慮するように修正
...
# ログファイルの設定
log_filename = f"model_comparison_modified_uncertainty_bps_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
###【改変】### BPSモデルのキャッシュファイル名を変更
BPS_GUIDE_FILE = 'trained_bps_guide_params_uncertainty.pt'
...
if os.path.exists(DATA_FILE):
    ...
    for i in trange(N,desc="Generating data for each person"):
        ...
        for t in range(Nt):
            ...
            if current_state == 1: eta_true_history[i, t, 0:3] = eta_t.squeeze()
            else: eta_true_history[i, t, 3] = eta_t.squeeze()
            eta_history_i=eta_t
    print("Simulation data generated.")
    print(f"Saving simulation data to '{DATA_FILE}'...");torch.save({'Y_generated':Y_generated.cpu(),'actual_states':actual_states,'eta_true_history':eta_true_history.cpu()},DATA_FILE)
...
###【改変】### BPSモデル入力用に、yの予測平均(m)と予測分散(F)を返す関数を新設
def get_kalman_predictive_distribution(Y, b0, B1, Lambda1, Q, R):
    """
    Kalman Filterを実行し、各時刻のyの一期先予測分布 (平均と分散) を返す。
    """
    N, Nt, O1 = Y.shape
    L1 = B1.shape[0]
    ...
    # 初期値
    eta_prev = torch.zeros(N, L1, 1, device=device)
    P_prev = torch.eye(L1, device=device).expand(N, -1, -1) * 1e3 # 初期分散を大きく設定
    ...
    for t in range(Nt):
        ...
        y_pred_cov = torch.bmm(torch.bmm(Lambda1.expand(N, -1, -1), P_pred), Lambda1.transpose(0, 1).expand(N, -1, -1)) + R
        ...
        # 数値安定性のためのジッター
        y_pred_cov.diagonal(dim1=-2, dim2=-1).add_(1e-6)

        K_t = torch.bmm(torch.bmm(P_pred, Lambda1.transpose(0, 1).expand(N, -1, -1)), torch.linalg.pinv(y_pred_cov))
        eta_updated = eta_pred + torch.bmm(K_t, v_t)
        P_updated = torch.bmm((torch.eye(L1, device=device).expand(N, -1, -1) - torch.bmm(K_t, Lambda1.expand(N, -1, -1))), P_pred)
...
###【改変】### 新しいヘルパー関数を使い、予測平均(m)と予測分散(F)を事前計算
print("Pre-calculating predictive distributions (mean and cov) for BPS model input...")
with torch.no_grad():
    ...
    # (N, Nt, O, J) と (N, Nt, O, O, J) の形にスタックする
...
T = Nt; gamma = 0.05; time_points = torch.arange(T, device=device, dtype=torch.float32)
pi_t = 1 + gamma - (1 - time_points / (T - 1))**2; pi_t = (pi_t / pi_t.sum()) * T


###【改変】### BPSモデルの定義を修正
def bps_random_walk_model_uncertainty(y_obs, m_factors, v_factors, time_weights):
    """
    BPSモデル。ベースラインモデルの予測の不確実性を考慮するバージョン。
    y_obs: (N, Nt, O)
    m_factors: (N, Nt, O, J) - 各モデルのy予測平均
    v_factors: (N, Nt, O, O, J) - 各モデルのy予測分散共分散
    """
    ...
    with pyro.plate("individuals", N, dim=-2):
        # --- 時点t=0の初期値 ---
        initial_loc = torch.tensor([5.0, 0.0], device=device) # state 1を優先
        ...
        for t in range(Nt):
            ...
            # --- ここからが主要な改変点 ---
            # スライドの式(3) f_t ~ N(m_t, v_t) に基づき、ファクターをサンプリング
            # このステップがベースラインモデルの不確実性をBPSに組み込む
            m_t = m_factors[:, t, :, :] # Shape: (N, O, J)
            v_t = v_factors[:, t, :, :, :] # Shape: (N, O, O, J)

            # pyro.plate を使って J個のモデルから独立にサンプリング
            with pyro.plate(f"models_{t}", J, dim=-1):
                # loc shape must be (N, J, O), cov shape must be (N, J, O, O)
                sampled_factors_t_permuted = pyro.sample(
                    f"factors_{t}",
                    dist.MultivariateNormal(m_t.permute(0, 2, 1), v_t.permute(0, 3, 1, 2))
                )
            # (N, O, J) の形に戻す
            sampled_factors_t = sampled_factors_t_permuted.permute(0, 2, 1)

            # スライドの式(1) y_t = alpha_t + beta_t^T @ f_t
            # einsumで個人ごとに内積を計算: (N, J) と (N, O, J) -> (N, O)
            mixed_factor = torch.einsum('nij,noj->no', beta_t, sampled_factors_t)
            y_mean = alpha_t + mixed_factor

            # 観測データの尤度を計算し、時間重み付けしてモデルに追加
            log_prob_t = dist.MultivariateNormal(y_mean, Sigma).log_prob(y_obs[:, t, :])
            pyro.factor(f"obs_factor_{t}", time_weights[t] * log_prob_t)

# --- SVIのセットアップ (モデル名を変更) ---
guide = AutoDiagonalNormal(bps_random_walk_model_uncertainty)
optimizer = Adam({"lr": 0.005})
svi = SVI(bps_random_walk_model_uncertainty, guide, optimizer, loss=Trace_ELBO())
...
if os.path.exists(BPS_GUIDE_FILE):
    ...
    loss_bps = -9999
    # Run the guide once to initialize its parameters after loading
    guide(Y_generated, m_factors, v_factors, time_weights=pi_t)
else:
    ...
    for step in pbar:
        ###【改変】### SVIステップ関数に新しい入力 (m_factors, v_factors) を渡す
        loss = svi.step(Y_generated, m_factors, v_factors, time_weights=pi_t) / N
...
if os.path.exists(RS_KF_MODEL_FILE):
    ...
    print(f"RS-KF model training finished. Duration: {duration_rs:.2f}s");print(f"Saving RS-KF model results to '{RS_KF_MODEL_FILE}'...");
    ###【改変】###
    results_to_save={'model_state_dict':best_model_rs_state,'loss':loss_rs,'duration':duration_rs};torch.save(results_to_save,RS_KF_MODEL_FILE)
...
with torch.no_grad():
    ...
    # Add .squeeze(1) to remove the extra dimension from each beta_t tensor
    estimated_logits = torch.stack([median_params[f'beta_{t}'].squeeze(1) for t in range(Nt)]).permute(1, 0, 2)
    # 重みを計算
    estimated_weights_bps = torch.nn.functional.softmax(estimated_logits, dim=-1).cpu().numpy()

    # yの予測値を計算: 重みとファクターの「平均値」を混合する
    # 予測時には確率的なサンプリングは行わず、期待値を用いるのが一般的
    m_factors_numpy = m_factors.cpu().numpy()
    # y_pred = w_1 * m_1 + w_2 * m_2
    y_pred_mixed_bps = (
        estimated_weights_bps[..., 0, np.newaxis] * m_factors_numpy[..., 0] +
        estimated_weights_bps[..., 1, np.newaxis] * m_factors_numpy[..., 1]
    )
...
print(f"{'BPS Random Walk (Uncertainty)':<30} | {loss_bps:<15.4f} | {rmse_bps:<15.4f} | {sens_bps:<15.4f} | {spec_bps:<15.4f} | {duration_bps:<15.2f} |")
...
predictive = Predictive(bps_random_walk_model_uncertainty, guide=guide, num_samples=1000,
                        return_sites=[f"beta_{t}" for t in range(Nt)])

###【改変】### Predictiveの実行時にも新しい入力を渡す
posterior_samples = predictive(Y_generated, m_factors, v_factors, time_weights=pi_t)
print("Generated 1000 samples from the posterior distribution of beta.")

# .squeeze(2) を追加して、各beta_tサンプルから余分な次元を削除
beta_samples = torch.stack([posterior_samples[f'beta_{t}'].squeeze(2) for t in range(Nt)], dim=2)
...
# ----------------------------------------------------------------
# Part 9: 個人レベルの可視化 (全員分・複数ファイルに分割) (変更なし)
# ----------------------------------------------------------------
# (このセクションのコードは長いため省略しますが、変更はありません)
