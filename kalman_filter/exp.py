...
# +【ご依頼による修正】BPSモデルをSVIからフルスクラッチMCMC実装に変更
...
# ログファイルの設定
log_filename = f"model_comparison_mcmc_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
# ▼▼▼【MCMC修正点】BPSモデルのキャッシュファイル名をMCMC用に変更 ▼▼▼
BPS_MCMC_SAMPLES_FILE = 'trained_bps_mcmc_samples.pt'
...
# ----------------------------------------------------------------
# Part 4: BPSモデルの定義と学習 (MCMCによる実装)
# ----------------------------------------------------------------
print("\n--- 4. Defining and Training BPS Random-Walk Model via MCMC ---")
...
# ▼▼▼【MCMC修正点】ここから下をMCMC実装に総入れ替え ▼▼▼

class MCMC_BPS:
    def __init__(self, Y, factors, time_weights, num_samples=2000, burn_in=1000):
        self.Y = Y
        self.factors = factors
        self.time_weights = time_weights
        self.N, self.Nt, self.O, self.J = factors.shape
        self.num_samples = num_samples
        self.burn_in = burn_in
        self.total_iter = num_samples + burn_in
        
        # Hyperparameters for Inverse Gamma priors
        self.a0 = 0.01; self.b0 = 0.01

        # Initialize parameters
        self.beta = torch.zeros(self.N, self.Nt, self.J, device=device)
        self.alpha = torch.zeros(self.N, self.Nt, self.O, device=device)
        self.Sigma_inv = torch.stack([torch.eye(self.O, device=device) for _ in range(self.N)])
        self.V_time_inv = torch.eye(self.J, device=device)
        self.V_person_inv = torch.eye(self.J, device=device)
        self.tau_time_sq_inv = torch.ones(self.O, device=device)
        self.tau_person_sq_inv = torch.ones(self.O, device=device)
        
        # Storage for posterior samples
        self.beta_samples = torch.zeros(self.num_samples, self.N, self.Nt, self.J, device=device)

    def _sample_variance(self, sum_sq, n_obs, a0=None, b0=None):
        """Helper to sample from Inverse Gamma posterior."""
        if a0 is None: a0 = self.a0
        if b0 is None: b0 = self.b0
        a_post = a0 + n_obs / 2.0
        b_post = b0 + sum_sq / 2.0
        return torch.distributions.Gamma(a_post, b_post).sample().reciprocal()

    def _ffbs_sample(self, y_res, F, V_inv, tau_sq_inv, initial_mean, initial_cov):
            """Forward-Filter Backward-Sample for a single time series (e.g., one person)."""
            T, K, D = F.shape # Nt, O, J for beta; Nt, O, O for alpha
            
            # Kalman Filter (Forward Pass)
            m_filtered, C_filtered = [], []
            m_pred, C_pred = [], []
            
            # t = 0
            m_t_pred = initial_mean
            C_t_pred = initial_cov
            m_pred.append(m_t_pred); C_pred.append(C_t_pred)

            F_t = F[0]
            y_t_res = y_res[0].unsqueeze(-1)
            Sigma_t_inv = torch.diag(tau_sq_inv)
            
            K_t = C_t_pred @ F_t.T @ torch.linalg.pinv(F_t @ C_t_pred @ F_t.T + torch.diag(1.0/tau_sq_inv))
            m_t_filt = m_t_pred + (K_t @ (y_t_res - F_t @ m_t_pred.unsqueeze(-1))).squeeze(-1)
            C_t_filt = (torch.eye(D, device=device) - K_t @ F_t) @ C_t_pred
            
            # ▼▼▼【修正】前方フィルターの数値安定化 (t=0) ▼▼▼
            C_t_filt = (C_t_filt + C_t_filt.T) / 2.0
            C_t_filt = C_t_filt + torch.eye(D, device=device) * 1e-6
            # ▲▲▲【修正】ここまで ▲▲▲

            m_filtered.append(m_t_filt); C_filtered.append(C_t_filt)
            
            for t in range(1, T):
                # Prediction
                m_t_pred = m_filtered[t-1]
                C_t_pred = C_filtered[t-1] + torch.linalg.pinv(V_inv)
                m_pred.append(m_t_pred); C_pred.append(C_t_pred)
                
                # Update
                F_t = F[t]
                y_t_res = y_res[t].unsqueeze(-1)
                K_t = C_t_pred @ F_t.T @ torch.linalg.pinv(F_t @ C_t_pred @ F_t.T + torch.diag(1.0/tau_sq_inv))
                m_t_filt = m_t_pred + (K_t @ (y_t_res - F_t @ m_t_pred.unsqueeze(-1))).squeeze(-1)
                C_t_filt = (torch.eye(D, device=device) - K_t @ F_t) @ C_t_pred
            ...
            # Backward Sampling
            samples = torch.zeros(T, D, device=device)
            samples[-1] = torch.distributions.MultivariateNormal(m_filtered[-1], C_filtered[-1]).sample()
            
            for t in range(T - 2, -1, -1):
                J_t = C_filtered[t] @ torch.linalg.pinv(C_pred[t+1])
                m_t_smooth = m_filtered[t] + J_t @ (samples[t+1] - m_pred[t+1])
                C_t_smooth = C_filtered[t] - J_t @ C_pred[t+1] @ J_t.T
                
                C_t_smooth = (C_t_smooth + C_t_smooth.T) / 2.0
                C_t_smooth = C_t_smooth + torch.eye(D, device=device) * 1e-6
                
                samples[t] = torch.distributions.MultivariateNormal(m_t_smooth, C_t_smooth).sample()
                
            return samples

    def run(self):
        pbar = tqdm(range(self.total_iter), desc="[BPS MCMC Sampling]", file=original_stdout)        
        for i in pbar:
            # 1. Sample beta for each person
            y_res_for_beta = self.Y - self.alpha
            for n in range(self.N):
                self.beta[n,:,:] = self._ffbs_sample(
                    y_res = y_res_for_beta[n],
                    F = self.factors[n],
                    V_inv = self.V_time_inv,
                    tau_sq_inv = torch.diagonal(self.Sigma_inv[n]),
                    initial_mean = torch.tensor([5.0, 0.0], device=device),
                    initial_cov = torch.linalg.pinv(self.V_person_inv)
                )

            # 2. Sample alpha for each person
            y_res_for_alpha = self.Y - torch.einsum('ntoj,ntj->nto', self.factors, self.beta)
            for n in range(self.N):
                 self.alpha[n,:,:] = self._ffbs_sample(
                    y_res = y_res_for_alpha[n],
                    F = torch.eye(self.O, device=device).expand(self.Nt, -1, -1),
                    V_inv = torch.diag(self.tau_time_sq_inv),
                    tau_sq_inv = torch.diagonal(self.Sigma_inv[n]),
                    initial_mean = torch.zeros(self.O, device=device),
                    initial_cov = torch.diag(1.0/self.tau_person_sq_inv)
                )

            # 3. Sample Sigma (observation variance) for each person
            residuals = self.Y - torch.einsum('ntoj,ntj->nto', self.factors, self.beta) - self.alpha
            
            # ▼▼▼【修正】einsumの次元指定を 'o' から 't' に修正 ▼▼▼
            # Apply time weighting to residuals by summing over the time 't' dimension
            weighted_sq_residuals = torch.einsum('t,nto->no', self.time_weights, residuals**2)
            
            for n in range(self.N):
                # ▼▼▼【修正】観測数を self.Nt に修正 ▼▼▼
                # We have O variances, each calculated from Nt observations
                variances = self._sample_variance(weighted_sq_residuals[n], self.Nt)
                self.Sigma_inv[n] = torch.diag(1.0 / variances)

            # 4. Sample V_time (random walk variance for beta)
            beta_diffs = self.beta[:, 1:, :] - self.beta[:, :-1, :]
            sum_sq_beta_diffs = torch.sum(beta_diffs**2, dim=[0, 1])
            v_time_variances = self._sample_variance(sum_sq_beta_diffs, self.N * (self.Nt - 1))
            self.V_time_inv = torch.diag(1.0 / v_time_variances)

            # 5. Sample V_person (initial state variance for beta)
            beta_t0_diffs = self.beta[:, 0, :] - torch.tensor([5.0, 0.0], device=device)
            sum_sq_beta_t0_diffs = torch.sum(beta_t0_diffs**2, dim=0)
            v_person_variances = self._sample_variance(sum_sq_beta_t0_diffs, self.N)
            self.V_person_inv = torch.diag(1.0 / v_person_variances)

            # 6. Sample tau_time_sq (random walk variance for alpha)
            alpha_diffs = self.alpha[:, 1:, :] - self.alpha[:, :-1, :]
            sum_sq_alpha_diffs = torch.sum(alpha_diffs**2, dim=[0, 1])
            tau_time_variances = self._sample_variance(sum_sq_alpha_diffs, self.N * (self.Nt - 1))
            self.tau_time_sq_inv = 1.0 / tau_time_variances
            
            # 7. Sample tau_person_sq (initial state variance for alpha)
            alpha_t0_diffs = self.alpha[:, 0, :]
            sum_sq_alpha_t0_diffs = torch.sum(alpha_t0_diffs**2, dim=0)
            tau_person_variances = self._sample_variance(sum_sq_alpha_t0_diffs, self.N)
            self.tau_person_sq_inv = 1.0 / tau_person_variances

            if i >= self.burn_in:
                self.beta_samples[i - self.burn_in] = self.beta.clone()
                
        print("MCMC sampling complete.")
        return self.beta_samples

if os.path.exists(BPS_MCMC_SAMPLES_FILE):
    print(f"Loading pre-computed BPS MCMC samples from '{BPS_MCMC_SAMPLES_FILE}'...")
    saved_mcmc = torch.load(BPS_MCMC_SAMPLES_FILE, map_location=device)
    bps_beta_samples = saved_mcmc['beta_samples']
    duration_bps = saved_mcmc.get('duration', 0.0)
else:
    print("Starting BPS model training with MCMC...")
    start_time_bps = time.time()
    
    mcmc_sampler = MCMC_BPS(Y_generated, factors_f, pi_t, num_samples=2000, burn_in=1000)
    bps_beta_samples = mcmc_sampler.run()
    
    duration_bps = time.time() - start_time_bps
    print(f"BPS model MCMC sampling finished. Duration: {duration_bps:.2f}s")
    print(f"Saving MCMC samples to '{BPS_MCMC_SAMPLES_FILE}'...")
    torch.save({'beta_samples': bps_beta_samples.cpu(), 'duration': duration_bps}, BPS_MCMC_SAMPLES_FILE)

loss_bps = -9999 # MCMC does not have a comparable loss value like ELBO or NLL
...
# --- 6a. BPSモデルの評価 ---
# ▼▼▼【MCMC修正点】SVIの評価ロジックをMCMCサンプルベースに変更 ▼▼▼
print("Evaluating BPS model (from MCMC samples)...");
with torch.no_grad():
    # 事後中央値を点推定値として使用
    estimated_logits = torch.median(bps_beta_samples.to(device), dim=0).values
    estimated_weights_bps = torch.nn.functional.softmax(estimated_logits, dim=-1).cpu().numpy()
    preds_3fac_numpy = preds_3fac_est.cpu().numpy()
    preds_1fac_numpy = preds_1fac_est.cpu().numpy()
    y_pred_mixed_bps = estimated_weights_bps[..., 0, np.newaxis] * preds_3fac_numpy + estimated_weights_bps[..., 1, np.newaxis] * preds_1fac_numpy
    rmse_bps = calculate_rmse(Y_generated.cpu(), torch.from_numpy(y_pred_mixed_bps))
    predicted_states_bps = (estimated_weights_bps[:, :, 1] > 0.5).astype(int)
...
print(f"{'BPS MCMC':<30} | {'N/A (MCMC)':<15} | {rmse_bps:<15.4f} | {sens_bps:<15.4f} | {spec_bps:<15.4f} | {duration_bps:<15.2f} |")
print(f"{'Regime-Switching KF':<30} | {loss_rs:<15.4f} | {rmse_rs:<15.4f} | {sens_rs:<15.4f} | {spec_rs:<15.4f} | {duration_rs:<15.2f} |")
print("Note: Loss for Baseline/RS-KF is NegLogLikelihood. Loss for BPS (MCMC) is not applicable.")
...
# --- 8a. BPSモデルの信頼区間を生成 ---
# ▼▼▼【MCMC修正点】SVIの信頼区間生成をMCMCサンプルベースに変更 ▼▼▼
print("\n--- 8a. Generating posterior samples for BPS model uncertainty ---")
prob_samples = torch.nn.functional.softmax(bps_beta_samples.to(device), dim=-1) # (n_samples, N, Nt, J)
prob_samples_s2 = prob_samples[..., 1] # (n_samples, N, Nt)
bps_lower_ci = torch.quantile(prob_samples_s2, 0.025, dim=0).cpu().numpy() # (N, Nt)
bps_upper_ci = torch.quantile(prob_samples_s2, 0.975, dim=0).cpu().numpy() # (N, Nt)
print("Calculated 95% credible intervals for BPS state probabilities from MCMC samples.")
...
for file_idx in range(num_files):
    ...
    for i, ind_id in enumerate(ids_to_plot):
        ...
        ax_bps.set_title(f'Individual #{ind_id+1} - BPS MCMC Model', fontsize=10)
    ...
    save_filename = f"individual_switches_plot_mcmc_part_{file_idx+1}.png"
...
for file_idx in range(num_files):
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])
    save_filename = f"eta_trajectories_filtered_mcmc_part_{file_idx+1}.png"
    plt.savefig(save_filename)
