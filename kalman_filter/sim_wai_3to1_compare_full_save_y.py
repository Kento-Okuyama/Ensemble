...
with Tee(file_path):
    ...
    if os.path.exists(data_file):
        ...
        data_to_save = {'Y_generated': Y_generated.cpu(), 'actual_states': actual_states, 'eta_true_history': eta_true_history.cpu()}
    ...
    num_runs = 1; learning_rate = 0.001; num_epochs = 10000; l2_decay = 0.01; patience = 100
    ...
    optimizer_3fac = torch.optim.AdamW(params_to_learn_3fac, lr=learning_rate, weight_decay=l2_decay)
    patience_counter = 0;
    ...
    optimizer_1fac = torch.optim.AdamW(params_to_learn_1fac, lr=learning_rate, weight_decay=l2_decay)
    patience_counter = 0
    for epoch in range(num_epochs):
    ...
    print("Baseline model estimation complete.")
    ...
    with torch.no_grad():
        ...
        # 予測値 y_hat をスタックして f_t を作成 (N, Nt, O, J)
    ...
    def bps_random_walk_model(y_obs, factors_f):
        ...
        beta_t0 = pyro.sample("beta_t0", dist.Normal(0., 1.).expand([J]).to_event(1))
        alpha_t0 = pyro.sample("alpha_t0", dist.Normal(0., 1.).expand([O]).to_event(1))
        
        beta_t = beta_t0; alpha_t = alpha_t0
        
        # 時間方向のループ
        for t in pyro.markov(range(Nt)):
            # 状態方程式 (ランダムウォーク)
            beta_t = pyro.sample(f"beta_{t}", dist.MultivariateNormal(beta_t, V))
            alpha_t = pyro.sample(f"alpha_{t}", dist.MultivariateNormal(alpha_t, torch.eye(O, device=device) * tau**2))
            
            # 観測方程式の平均を計算
            # y_mean = alpha + f * beta
            # einsum: 'ntoj,j->nto' -> (N, Nt, O, J) と (J,) から (N, Nt, O) を計算
            # プレートがあるので、(N, O, J) と (J,) から (N, O)
            factors_t = factors_f[:, t, :, :]
            y_mean = alpha_t.unsqueeze(0) + torch.einsum('noj,j->no', factors_t, beta_t)

            with pyro.plate("individuals", N):
                pyro.sample(f"obs_{t}", dist.MultivariateNormal(y_mean, Sigma).to_event(0), obs=y_obs[:, t, :])
    ...
    for n in range(N):
        y_preds_bps[n, :, :] = estimated_alphas + np.einsum('toj,tj->to', factors_f_numpy[n, :, :, :], estimated_betas)
    
    rmse_bps = calculate_rmse(Y_generated, torch.tensor(y_preds_bps, device=device))

    # Sens/Specを計算するための状態分類
    predicted_states_bps_agg = (estimated_betas[:, 1] > estimated_betas[:, 0]).astype(int) + 1 # 1 or 2
    ...
    ax1.axhline(0.5, color='grey', linestyle='--', label='Equal Weight (0.5)')
