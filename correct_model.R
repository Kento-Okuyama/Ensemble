...
model {
  ...
  for (n in 1:N) {
    for (t in 2:Nt) {
      ...
      if (k == 1) {
        y[n, t] ~ normal((1 - ar_pos) * target_mean + ar_pos * y[n, t-1], sigma[k]);
      } else if (k == 2) {
        y[n, t] ~ normal((1 - ar_neg) * target_mean + ar_neg * y[n, t-1], sigma[k]);
      } else if (k == 3) {
...
generated quantities {
  ...
  for (n in 1:N) {
    for (t in 2:Nt) {
      ...
      if (k == 1) {
        y_rep[n, t-1] = normal_rng((1 - ar_pos) * target_mean + ar_pos * y[n, t-1], sigma[k]);
        log_lik[n, t-1] = normal_lpdf(y[n, t] | (1 - ar_pos) * target_mean + ar_pos * y[n, t-1], sigma[k]);
      } else if (k == 2) {
        y_rep[n, t-1] = normal_rng((1 - ar_neg) * target_mean + ar_neg * y[n, t-1], sigma[k]);
        log_lik[n, t-1] = normal_lpdf(y[n, t] | (1 - ar_neg) * target_mean + ar_neg * y[n, t-1], sigma[k]);
      } else if (k == 3) {
