fit_apriori <- function(data, iter = 2000, chains = 4, refresh = 0) {
  ...
  generated quantities {
    ...
    for (n in 1:N) {
      ...
      for (t in 1:train_Nt) {
        ...
        log_lik[n, t] = pi_t * normal_lpdf(train_y[n, t] | train_eta[n, t], sigma_m);
      }
  ...
  combined_predictions <- function(res_ar, res_ma, data) {
    ...
    val_e <- array(c(e1[(data$train_Nt + 1):(data$train_Nt + data$val_Nt)], 
                     e2[(data$train_Nt + 1):(data$train_Nt + data$val_Nt)]), dim = c(data$N, data$val_Nt, 2))
    test_e <- array(c(e1[(data$train_Nt + data$val_Nt + 1):(data$train_Nt + data$val_Nt + data$test_Nt)], 
                      e2[(data$train_Nt + data$val_Nt + 1):(data$train_Nt + data$val_Nt + data$test_Nt)]), dim = c(data$N, data$test_Nt, 2))
    
    pred_f <- array(c(f1, f2), dim = c(data$N, data$train_Nt + data$val_Nt + data$test_Nt, 2))
    ...
    return(list(pred_e = pred_e, pred_f = pred_f, train_f = train_e, val_e = val_e, test_e = test_e, train_f = train_f, val_f = val_f, test_f = test_f))
  }
  ...
  data_fit <- list(
    ...
    train_e = predictions$train_e,
    val_e = predictions$val_e,
    test_e = predictions$test_e,
