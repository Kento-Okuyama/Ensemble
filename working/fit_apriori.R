fit_apriori <- function(data, iter = 2000, chains = 4, refresh = 0) {
  ...
  model {
    ...
    mu_ar ~ normal(0, 1);
    sigma_ar ~ cauchy(0, 1e-1);
    sigma_m ~ cauchy(0, 1e-1);
  ...
  generated quantities {
    ...
    for (n in 1:N) {
      ...
      for (t in 1:train_Nt) {
        ...
        log_lik[n, t] = pi_t * normal_lpdf(train_y[n, t] | lambda * train_eta[n, t], sigma_m);
      }
  ...
  combined_predictions <- function(res_ar, res_ma, data) {
    # Extract sampled eta and y predictions
    e1_samples <- res_ar$eta_pred  
    e2_samples <- res_ma$eta_pred  
    f1_samples <- res_ar$y_pred    
    f2_samples <- res_ma$y_pred    
    
    pred_e <- list( 
      sample = list(ar = e1_samples, ma = e2_samples),
      train = array(c(e1_samples[,,1:data$train_Nt], e2_samples[,,1:data$train_Nt]), dim = c(iter * chains, data$N, data$train_Nt, 2)),
      val = array(c(e1_samples[,,(data$train_Nt + 1):(data$train_Nt + data$val_Nt)], 
                    e2_samples[,,(data$train_Nt + 1):(data$train_Nt + data$val_Nt)]), 
                  dim = c(iter * chains, data$N, data$val_Nt, 2)),
      test = array(c(e1_samples[,,(data$train_Nt + data$val_Nt + 1):(data$train_Nt + data$val_Nt + data$test_Nt)], 
                     e2_samples[,,(data$train_Nt + data$val_Nt + 1):(data$train_Nt + data$val_Nt + data$test_Nt)]), 
                   dim = c(iter * chains, data$N, data$test_Nt, 2))
      )
    
    pred_f <- list( 
      sample = list(ar = f1_samples, ma = f2_samples),
      train = array(c(f1_samples[,,1:data$train_Nt], f2_samples[,,1:data$train_Nt]), dim = c(iter * chains, data$N, data$train_Nt, 2)),
      val = array(c(f1_samples[,,(data$train_Nt + 1):(data$train_Nt + data$val_Nt)], 
                    f2_samples[,,(data$train_Nt + 1):(data$train_Nt + data$val_Nt)]), 
                  dim = c(iter * chains, data$N, data$val_Nt, 2)),
      test = array(c(f1_samples[,,(data$train_Nt + data$val_Nt + 1):(data$train_Nt + data$val_Nt + data$test_Nt)], 
                     f2_samples[,,(data$train_Nt + data$val_Nt + 1):(data$train_Nt + data$val_Nt + data$test_Nt)]), 
                   dim = c(iter * chains, data$N, data$test_Nt, 2))
    )
    
    return(list(pred_e = pred_e, pred_f = pred_f))
  }
  ...
  data_fit <- list(
    ...
    train_f = predictions$pred_f$train,
    val_f = predictions$pred_f$val,
    test_f = predictions$pred_f$test
  )
