DGP <- function(N = 5, Nt = 30, seed = 123, train_ratio = 0.6, val_ratio = 0.2) {
  ...
  for (p in 1:N) {
    ...
    for (t in 2:Nt) {
      ...
      if (current_regime == 1) {
        eta[p, t] <- (1 - ar_coef) * target_mean + ar_coef * eta[p, t - 1] + rnorm(1, 0, sigma_s[current_regime])
      } else if (current_regime == 2) {
        eta[p, t] <- target_mean + ma_coef * (eta[p, t - 1] - target_mean) + rnorm(1, 0, sigma_s[current_regime])
      } else {
