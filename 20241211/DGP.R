DGP <- function(N = 5, Nt = 30, seed = 123, train_ratio = 0.6, val_ratio = 0.2) {
  ...
  mu <- c(3, -3, 0)           # Means for each regime
  ...
  for (p in 1:N) {
    ...
    eta[p, 1] <- rnorm(1, mu[current_regime], sigma_s[current_regime])
    y[p, 1] <- lambda[current_regime] * eta[p, 1] + rnorm(1, 0, sigma_m)
    ...
    for (t in 2:Nt) {
      ...
      if (current_regime == 1) {
        eta[p, t] <- (1 - sum(ARma_coef)) * target_mean + 
          ...
          arMA_coef[1] * (eta[p, t - 1] - target_mean) + 
          arMA_coef[2] * ifelse(t > 2, (eta[p, t - 2] - target_mean), 0) + 
          arMA_coef[3] * ifelse(t > 3, (eta[p, t - 3] - target_mean), 0)
      } else if (current_regime == 2) {
        eta[p, t] <- target_mean + ma_coef[1] * (eta[p, t - 1] - target_mean) + 
          ma_coef[2] * ifelse(t > 2, (eta[p, t - 2] - target_mean), 0) + 
          rnorm(1, 0, sigma_s[current_regime])
      } else {
        eta[p, t] <- rnorm(1, target_mean, sigma_s[current_regime])
      }

      y[p, t] <- lambda[current_regime] * eta[p, t] + rnorm(1, 0, sigma_m)
    }
  }
