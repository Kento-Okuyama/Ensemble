fit_apriori_latent <- function(data, iter = 2000, chains = 4) {
  ...
  generated quantities {
    ...
    for (n in 1:N) {
      eta_pred[n, 1] = normal_rng(mu_ma, sigma_ma);
      y_pred_ma[n, 1] = normal_rng(eta_pred[n, 1], sigma_m);

      for (t in 2:(train_Nt + test_Nt)) {
        eta_pred[n, t] = normal_rng(mu_ma + ma_coef * (eta_pred[n, t - 1] - mu_ma), sigma_ma);
        y_pred_ma[n, t] = normal_rng(eta_pred[n, t], sigma_m);
      }
    }
  }
  "
  ...
  generated quantities {
    ...
    for (n in 1:N) {
      eta_pred[n, 1] = normal_rng(mu_wn, sigma_wn);
      y_pred_wn[n, 1] = normal_rng(eta_pred[n, 1], sigma_m);

      for (t in 2:(train_Nt + test_Nt)) {
        eta_pred[n, t] = normal_rng(mu_wn, sigma_wn);
        y_pred_wn[n, t] = normal_rng(eta_pred[n, t], sigma_m);
      }
    }
  }
  "
